...
public class PersistentLocalTimeAsString implements EnhancedUserType, Serializable {
    ...
    public Object nullSafeGet(ResultSet rs, String[] names, SessionImplementor session, Object owner)
        ...
        Object timestamp = StandardBasicTypes.STRING.nullSafeGet(rs, names[0], session, owner);
        if (timestamp == null) {
            return null;
        }
    ...
    public void nullSafeSet(PreparedStatement statement, Object value, int index, SessionImplementor session)
            throws HibernateException, SQLException {
        if (value == null) {
            StandardBasicTypes.STRING.nullSafeSet(statement, null, index, session);
        } else {
            LocalTime lt = ((LocalTime) value);
            StandardBasicTypes.STRING.nullSafeSet(statement, lt.toString(), index, session);
        }
    }
